...
public class VersaPlayerView: View, PIPProtocol {
    ...
    public func updateControls(toTime time: Double) {
    ...
    open func set(item: VersaPlayerItem?) {
        ...
        if controls?.isCurrentTimeChange == false{
            controls?.prepareNotificationListener()
        }
    ...
    public func playVideoQulity(item: VersaPlayerItem){
        
        if playbackDelegate?.playbackShouldBegin(player: player) ?? true {
            playbackDelegate?.playerPlay(player: player)
...
//
//            }else {
//                playbackDelegate?.playerStop(player: player)
//
//                player.pause()
...
//            }
    ...
    @IBAction open func play(sender: Any? = nil) {
        if playbackDelegate?.playbackShouldBegin(player: player) ?? true {
            ...
            player.replaceCurrentItem(with: player.videoPlayer)
            player.play()
            controls?.playPauseButton?.set(active: true)
            isPlaying = true
        }
