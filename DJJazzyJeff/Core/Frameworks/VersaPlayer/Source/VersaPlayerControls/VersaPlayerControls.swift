...
open class VersaPlayerControls: View {
    ...
    @IBOutlet public weak var btnVideoQulity: UIButton!

    @IBOutlet public weak var btnVolume: UIButton!
    @IBOutlet public weak var btnFullView: UIButton!
    @IBOutlet public weak var btnSubTitle: UIButton!
    @IBOutlet public weak var viewSlider: UIView!
    @IBOutlet public weak var btnNextShow: UIButton!
    
    @IBOutlet public weak var btnInfo: UIButton!
    @IBOutlet public weak var btnVideo: UIButton!
    @IBOutlet public weak var btnBrodCast: UIButton!
    @IBOutlet public weak var btnShare: UIButton!
    @IBOutlet public weak var btnFavorite: UIButton!
    

    /// VersaPlayer intance being controlled
    public weak var handler: VersaPlayerView!
    public var isCurrentTimeChange = true
...
//        Bundle.main.loadNibNamed("VersaPlayerControls", owner: self, options: nil)
//
//    }
    #if os(macOS)
    
    open override func touchesBegan(with event: NSEvent) {
        if behaviour.isOtherButtonClicked == false{
            behaviour.hide()
        }
    }
    ...
    open func timeStreemChange(toTime time: Double) {
        print("TIMEEEEE -- > \(time)")
    }
    open func timeDidChange(toTime time: Double) {
        print("isCurrentTimeChange == \(isCurrentTimeChange)")
        if isCurrentTimeChange == false{
            return
        }
        
        currentTimeLabel?.update(toTime: time)
        totalTimeLabel?.update(toTime: handler.player.endTime().seconds)
        setSeekbarSlider(start: handler.player.startTime().seconds, end: handler.player.endTime().seconds, at: time)
        
        if !(handler.isSeeking || handler.isRewinding || handler.isForwarding) {
            behaviour.update(with: time)
        }
    }
    
    public func setSeekbarSlider(start startValue: Double, end endValue: Double, at time: Double) {
        print("isCurrentTimeChange SEEKP BAR == \(isCurrentTimeChange)")
        if isCurrentTimeChange == false{
            return
        }
        ...
        DispatchQueue.main.async {
            if self.handler != nil{
                if self.handler.player != nil{
                    self.seekbarSlider?.minimumValue = Float(self.handler.player.startTime().seconds)
                    if self.handler.player.endTime().seconds != 0.0{
                        self.seekbarSlider?.maximumValue = Float(self.handler.player.endTime().seconds)
                    }
                }
                else{
                    self.seekbarSlider?.minimumValue = 0.0
                    DispatchQueue.main.async {
                        self.seekbarSlider?.maximumValue = 0.0
                    }
                }
            }
        }
    ...
    open func prepare() {
        ...
        airplayButton = MPVolumeView(frame: CGRect(x: -6, y: -6, width: 40, height: 40))
        airplayButton?.showsVolumeSlider = false
        btnBrodCast.setImage(UIImage(), for: .normal)
        btnBrodCast.addSubview(airplayButton!)
...
//        airplayButton?.frame = airplayContainer?.bounds ?? CGRect.zero
    ...
    open func prepareNotificationListener() {
        ...
        NotificationCenter.default.addObserver(forName: VersaPlayer.VPlayerNotificationName.assetLoaded.notification, object: nil, queue: OperationQueue.main) { [weak self] (notification) in
            guard let self = self else { return }
            if let time = notification.userInfo?[VersaPlayer.VPlayerNotificationInfoKey.time.rawValue] as? Double {
                self.checkOwnershipOf(object: notification.object, completion: self.timeStreemChange(toTime: time))
            }
        }
...
//        handler.player.seek(to: time)
        moveForward(setValue: skipSize)
    }
    
    public func moveForward(setValue : Double){
        let time = handler.player.currentTime() + CMTime(seconds: setValue, preferredTimescale: CMTimeScale(NSEC_PER_SEC))
        handler.player.seek(to: time)
    }
    ...
    @IBAction func allEpisodeBtnAction(_ sender: Any) {
        print("testing")
        //        self.allEpisodeBtnActionBlock?()
    }
}
